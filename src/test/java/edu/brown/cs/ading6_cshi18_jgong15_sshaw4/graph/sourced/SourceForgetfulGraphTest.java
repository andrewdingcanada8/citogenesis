...
/**
 * Unit testing forgetful SourcedGraph implementation
 */
public class SourceForgetfulGraphTest {
        ...
        if (l.isConnected(inNode)) {
          for (Node outNode : nodes) {
            if (l.isConnected(outNode) && inNode.compatibleWith(outNode)) {
              SourcedForgetfulVertex<Node, Line> outVert = new SourcedForgetfulVertex<>(outNode, this);
